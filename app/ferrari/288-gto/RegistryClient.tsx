...
export default function RegistryClient({ cars }: { cars: Submission[] }) {
  ...
  const pct = Math.round((documented / 272) * 100);
  ...
  return (
    <main style={{ background: '#080F1A', color: '#E2EEF7', fontFamily: 'Georgia, serif', minHeight: '100vh' }}>
      <header style={{ background: '#0A1828', borderBottom: '1px solid #1E3A5A', padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <nav style={{ display: 'flex', gap: '20px', fontSize: '13px', alignItems: 'center' }}>
      ...
      <section style={{ padding: '60px 40px 40px', borderBottom: '1px solid #1E3A5A' }}>
        ...
        <div style={{ display: 'flex', gap: '48px', marginTop: '40px' }}>
          {[
            ...
            { n: String(272 - documented), l: 'Undocumented' },
            { n: `${pct}%`, l: 'Complete' },
          ].map(s => (
            <div key={s.l}>
      ...
      <section style={{ padding: '24px 40px', borderBottom: '1px solid #1E3A5A', display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
        <input
          value={query}
          onChange={e => setQuery(e.target.value)}
          placeholder="Search chassis number..."
          style={{ background: '#0D1E36', border: '1px solid #1E3A5A', color: '#E2EEF7', padding: '10px 16px', fontSize: '14px', width: '280px', fontFamily: 'Georgia, serif', outline: 'none' }}
        />
        <select
          value={market}
