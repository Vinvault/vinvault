...
export async function GET(req: NextRequest) {
  ...
  const res = await fetch(`${url}/rest/v1/models?order=model.asc&limit=${limit}${filter}`, {
...
export async function PATCH(req: NextRequest) {
  ...
  const url = process.env.SUPABASE_URL;
  if (!url) return NextResponse.json({ error: "No config" }, { status: 500 });
  const res = await fetch(`${url}/rest/v1/models?id=eq.${id}`, {
    method: "PATCH",
    headers: { ...h(), Prefer: "return=representation" },
...
export async function DELETE(req: NextRequest) {
  ...
  const url = process.env.SUPABASE_URL;
  if (!url) return NextResponse.json({ error: "No config" }, { status: 500 });
  await fetch(`${url}/rest/v1/models?id=eq.${id}`, { method: "DELETE", headers: h() });
